...
class PVD:
    """
    Pixel Value Differencing - Uses LSB of first pixel in pair
    Simple and reliable LSB steganography
    """
    ...
    def embed(image, message):
        ...
        # Sequential pixel selection (use every pixel, not just pairs)
        h, w = image.shape
        ...
        return stego.reshape(image.shape).astype(np.uint8), "LSB"
